...
public class UsuarioDAO {
    ...
    public ArrayList<CanaisDTO> listarCanais2() {
        ...
        try {
            ...
            while (rs.next()) {
                CanaisDTO objcanaisDTO = new CanaisDTO();
                objcanaisDTO.setId(rs.getInt("id"));
                objcanaisDTO.setCanalamd(rs.getString("canalamd"));
                objcanaisDTO.setTipoadm(rs.getString("tipoadm"));
                objcanaisDTO.setAutentica(rs.getString("autentica"));
            ...
            JOptionPane.showMessageDialog(null, "listar2" + erro);
        }
        return lista;
    }
 ...
 public void excluirCadastros2(CanaisDTO objcanaisDTO){
      String sql = "delete from canaladm where id = ?";
      conn = new ConnectionFactory().getConnection();
...
public Vector  pesquisar(String pesq, String area) throws Exception {
     Vector  tb = new Vector ();
     
     conn = new ConnectionFactory().getConnection();
     String sql = "select * from canais where " +  area  +" like '" + pesq + "%'";
...
public Vector  pesquisarCanal(String pesq, String area) throws Exception {
     Vector  tb = new Vector ();
     
     conn = new ConnectionFactory().getConnection();
     String sql = "select * from canaladm where " +  area  +" like '" + pesq + "%'";
     PreparedStatement ps = (PreparedStatement) conn.prepareStatement(sql);
     ...
     while(rs.next()){
         Vector nl = new Vector();
         nl.add(rs.getInt("id"));
         nl.add(rs.getString("canalamd"));
         nl.add(rs.getString("tipoadm"));
         nl.add(rs.getString("autentica"));
